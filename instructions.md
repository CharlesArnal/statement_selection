...
The textbooks are found in folders with names such as mit_books/algebra_i_student_notes or mit_books/algebraic_geometry_i. Each folder corresponds to a single textbook. The folder may contain `.tex`, `.md` (from OCR), and/or `.pdf` files. When reading a textbook, prefer `.tex` files first, then `.md`, then `.pdf`.
